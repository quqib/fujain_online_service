...
class ServiceCounterServiceInfo(Base):
    ...
    handling_time = Column(String(2000), nullable=True, comment='办理时间')

    # 办理地点（详细地址）
    service_location = Column(String(2000), nullable=True, comment='办理地点')

    # 交通指引（可包含公交、地铁、导航提示等）
    transportation_guide = Column(Text, nullable=True, comment='交通指引')

    # 受理条件
    acceptance_criteria = Column(Text, nullable=True, comment='受理条件')
